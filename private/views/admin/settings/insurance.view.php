...
<div class="col-lg-12">
  ...
  <div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
      <h3 class="mb-0">Insurance Companies</h3> <!-- Use `mb-0` to remove default margin -->
      <a href="<?=ROOT ?>/Insurances/add">
      ...
      <table class="table table-hover table-stripped">
        ...
        <tbody>
          <?php if ($data): ?>
            ...
            <tr>
              <th scope="row"><?= $num ?></th>
              <td><?=strtoupper($row->insuranceName) ?></td>
              <td><?=ucwords($row->userInfo->fullname)?></td>
              <td class="text-center"><?=get_date($row->date) ?></td>
              <td class="text-end">
                <a href="<?=ROOT?>/insurances/edit/<?=$row->id?>" class="text-primary"><i class="fa fa-edit"></i></a>|
                <a href="<?=ROOT?>/insurances/delete/<?=$row->id?>" class="text-danger"><i class="fa fa-trash-alt"></i></a>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php else: ?>
