...
class Test extends Model
{
  ...
  public function getUserById($rows)
  {
    ...
    if (!empty($rows[0]->userId))
    {
      ...
      {
        $query = "SELECT id, firstname,lastname,username FROM users WHERE id = :id LIMIT 1";
  ...
  {
    $db = new Database();
    if (!empty($rows[0]->testsLabSectionId))
    {
      foreach ($rows as $key => $row)
      {
        $query = "SELECT * FROM labsections WHERE id = :id LIMIT 1";
        $labSec = $db->query($query,['id'=>$row->testsLabSectionId]);
        if (!empty($labSec))
        {
  ...
  public function getUnitById($rows)
  {
    ...
    if (!empty($rows[0]->unitid))
    {
      foreach ($rows as $key => $row)
      {
        ...
        $unit = $db->query($query,['unitId'=>$row->unitid]);
        if (!empty($unit))
        {
